...
db.session.add(ArgumentPost("The federal government should impose a national $15/hr minimum wage.", False, True, "A $15/hr minimum wage would hurt society\'s most disadvantaged people looking for work even if it doesn\'t increase overall unemployment.", "If I\'m an employer who is now forced to hire at a higher wage then I will expect greater productivity to off-set the additional cost. Therefore, I will be more likely to hire an older experienced worker who came out of retirement to work for the higher wage instead of a troubled minority youth who is in desperate need of work.", 4))     
...
db.session.add(ArgumentPost("The federal government should impose a national $15/hr minimum wage.", False, False, "Just expand the earned income tax credit (EITC) instead!", "The EITC is far more effective at alleviating poverty since it's a direct cash subsidy instead of a dangerously high price floor which is what a $15/hr minimum wage is.", 5))
db.session.add(ArgumentPost("The federal government should impose a national $15/hr minimum wage.", True, False, "I would rather deal with robots instead of human cashiers anyways.", "All other things equal, a higher minimum wage will encourage firms to substitute capital for labor. This will accelerate technological progress in our everyday lives!", 5))
...
# arg_id, value, voter_id
db.session.add(Vote(2, 8, 1))
db.session.add(Vote(3, 5, 1))
db.session.add(Vote(4, 9, 1))
db.session.add(Vote(9, 8, 1))
db.session.add(Vote(1, 1, 2))
db.session.add(Vote(3, 4, 2))
db.session.add(Vote(4, 10, 2))
db.session.add(Vote(9, 9, 2))
db.session.add(Vote(5, 3, 3))
db.session.add(Vote(6, 10, 3))
db.session.add(Vote(8, 4, 3))
db.session.add(Vote(10, 8, 3))
db.session.add(Vote(5, 3, 4))
db.session.add(Vote(6, 9, 4))
db.session.add(Vote(7, 4, 4))
db.session.add(Vote(10, 7, 4))
db.session.add(Vote(1, 3, 5))
db.session.add(Vote(2, 9, 5))
db.session.add(Vote(3, 4, 5))
db.session.add(Vote(4, 10, 5))
#commit
db.session.commit()